...
public abstract class WohnungSearcher
    ...
    public void run() {
        try {
            ...
            if (dialog.open() == Window.OK) {
                ...
                adopt( Iterables.find( content, new Predicate<WohnungComposite>() {
    ...
    class WohnungTableDialog
        ...
        protected Control createDialogArea( Composite parent ) {
            ...
            setTitle( "Wohnungen suchen" );
            setMessage( "Bitte geben Sie in Ziffern Objektnummer, -fortführung, Gebäudenummer, -fortführung, Wohnungsnummer oder -fortführung ein und klicken Sie auf Suchen. Anschließend können Sie aus den maximal 50 Ergebnissen eines wählen, das mit Klick auf Ok dem Flurstück zugewiesen wird." );
            ...
            ActionButton search = new ActionButton( composite, new Action( "Suchen" ) {
                ...
                public void run() {
                    ...
                    if (text != null && !text.isEmpty()) {
                        Integer asInteger = Integer.parseInt( text );
                        BooleanExpression q = QueryExpressions.eq( template.objektFortfuehrung(), asInteger );
                    ...
                    if (text != null && !text.isEmpty()) {
                        Integer asInteger = Integer.parseInt( text );
                        BooleanExpression q = QueryExpressions.eq( template.gebaeudeFortfuehrung(), asInteger );
                        expr = expr == null ? q : QueryExpressions.and( expr,  q );
                    }                    
                    text = t5.getText();
                    if (text != null && !text.isEmpty()) {
            ...
            viewer.addColumn( new DefaultFeatureTableColumn( prop ).setHeader( "Objekt" ) );
            prop = new PropertyDescriptorAdapter( type.getProperty( "objektFortfuehrung" ) );
            viewer.addColumn( new DefaultFeatureTableColumn( prop ).setHeader( "-fortf." ) );
            prop = new PropertyDescriptorAdapter( type.getProperty( "gebaeudeNummer" ) );
            viewer.addColumn( new DefaultFeatureTableColumn( prop ).setHeader( "Gebäude" ) );
            ...
            viewer.addColumn( new DefaultFeatureTableColumn( prop ).setHeader( "Wohnung" ) );
            ...
            viewer.addColumn( new DefaultFeatureTableColumn( prop ).setHeader( "Flurstück" ).setWeight( 2, 50 ) );
