...
public class MdbImportBewertungenOperation
    ...
    protected IStatus doExecute0( IProgressMonitor monitor, IAdaptable info )
        ...
        try {
            ...
            sub = new SubMonitor( monitor, 10 );
            importEntity( db, monitor, NHK2010BaupreisIndexComposite.class,
                    new EntityCallback<NHK2010BaupreisIndexComposite>() {
                ...
                if (eingangsnummer != null) {
                    ...
                    if (anbauten != null && !anbauten.isEmpty()) {
                        ...
                        for (String anbaute : anbautenEinzeln) {
                            ...
                            NHK2010AnbautenComposite anbauTemplate = QueryExpressions
                                    .templateFor( NHK2010AnbautenComposite.class );
    ...
    private void importModernisierungsgrad( Database db, IProgressMonitor monitor, final File parentFolder )
        ...
        while ((builderRow = table.getNextRow()) != null) {
            ...
            if ("frm_wwohnneu".equals( type ) || "frm_sachwnhk2010".equals( type ) || "frm_ertragswertn".equals( type )) {
                ...
                Double eingangsnummer = (Double)builderRow.get( "EINGANGSNR" );
                VertragComposite vertrag = null;
                if (eingangsnummer != null) {
                    VertragComposite vertragTemplate = QueryExpressions.templateFor( VertragComposite.class );
                    BooleanExpression expr = QueryExpressions.eq( vertragTemplate.eingangsNr(),
                            eingangsnummer.intValue() );
                    vertrag = KapsRepository.instance().findEntities( VertragComposite.class, expr, 0, 1 ).find();
                }
                if (vertrag == null) {
                    // 10.0 gibts nicht, sicher ein gelöschter Testvertrag
                    ...
                    if ("frm_sachwnhk2010".equals( type )) {
                        NHK2010BewertungComposite nhk2010 = NHK2010BewertungComposite.Mixin.forVertrag( b.vertrag()
                                .get() );
                        if (nhk2010 == null) {
        ...
        while ((builderRow = table.getNextRow()) != null) {
            ...
            if (vertrag == null) {
                ...
                if ("frm_buch".equals( maske )) {
                    ...
                    boolean found = false;
                    VertragsdatenBaulandComposite bc = VertragsdatenBaulandComposite.Mixin.forVertrag( vertrag );
                    if (bc != null) {
                        bc.bemerkungen().set( text );
                    }
                    else {
                        wmvaopfW.write( "no VertragsdatenBaulandComposite found for " + eingangsnummer + "\n" );
                    }
                }
                else if ("frm_gveditagrar".equals( maske )) {
                    boolean found = false;
                    VertragsdatenAgrarComposite bc = VertragsdatenAgrarComposite.Mixin.forVertrag( vertrag );
                    if (bc != null) {
                    ...
                    if (ec != null) {
                        ...
                        wmvaopfW.write( "no ErtragswertverfahrenComposite found for " + eingangsnummer + "\n" );
                    }
                }
                else {
